...
if(!empty($_FILES['photo']['tmp_name'])){
    ...
    $src_info['rate']=imagesy($src_img)/imagesx($src_img);//計算寬高比
    if($src_info['rate']<=1){
    ...
    $dst_info['width']=200;
    $dst_info['height']=200;
    $white=imagecolorallocate($dst_img,255,255,255); //定義顏色
    imagefill($dst_img,0,0,$white);//填入顏色
}
...
<hr>
<div>
<img src="<?="./img/".$_FILES['photo']['name'];?>" style="width:200px;">
</div>
<!----縮放圖形----->
<h3>縮放圖形</h3>
<?php
